<?php /* Smarty version 3.1.27, created on 2022-10-06 01:49:05
         compiled from "/home/u653868903/domains/hybridminers.io/public_html/tmpl/mfooter.tpl" */ ?>
<?php
/*%%SmartyHeaderCode:1819346652633e3411afbdb8_69741910%%*/
if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'f56d972cd1edc302bcb5efa51e214430687eb554' => 
    array (
      0 => '/home/u653868903/domains/hybridminers.io/public_html/tmpl/mfooter.tpl',
      1 => 1664187453,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '1819346652633e3411afbdb8_69741910',
  'has_nocache_code' => false,
  'version' => '3.1.27',
  'unifunc' => 'content_633e3411afcf67_35679577',
),false);
/*/%%SmartyHeaderCode%%*/
if ($_valid && !is_callable('content_633e3411afcf67_35679577')) {
function content_633e3411afcf67_35679577 ($_smarty_tpl) {

$_smarty_tpl->properties['nocache_hash'] = '1819346652633e3411afbdb8_69741910';
?>
  
</div>

  
  
        <!-- footer section start -->
        <footer class="footer bg_img" data-background="assets/images/bg/bg-7.jpg">
            <div class="footer__top">
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-lg-12 text-center">
                            <a href="#0" class="footer-logo"><img src="assets/images/logo.png" alt="image"></a>
                            <ul class="footer-short-menu d-flex flex-wrap justify-content-center mt-4">
                                <li><a href="#0">Home</a></li>
                                <li><a href="#0">Privacy & Policy</a></li>
                                <li><a href="#0">Terms & Conditions</a></li>
                            </ul>
                            <p>© 2022 <a href="?a=home" class="base--color"> Hybridminers</a>. All rights reserved</p>
                            <div class="col-md-6">
                            <ul class="social-link-list d-flex flex-wrap justify-content-md-end justify-content-center">
                                <li><a href="#0" data-toggle="tooltip" data-placement="top" title="facebook"><i class="lab la-facebook-f"></i></a></li>
                                <li><a href="#0" data-toggle="tooltip" data-placement="top" title="twitter"><i class="lab la-twitter"></i></a></li>
                                <li><a href="#0" data-toggle="tooltip" data-placement="top" title="pinterest"><i class="lab la-pinterest-p"></i></a></li>
                                <li><a href="#0" data-toggle="tooltip" data-placement="top" title="pinterest"><i class="lab la-pinterest-in"></i></a></li>
                            </ul>
                        </div>
                        </div>
                    </div>
                </div>
            </div>
            
        </footer>
        <!-- footer section end -->
    </div>
    
    <!-- CORE JS FRAMEWORK - START -->
    <?php echo '<script'; ?>
 src="dashboard/assets/js/jquery.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="dashboard/assets/js/jquery.easing.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="dashboard/assets/plugins/bootstrap/js/bootstrap.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="dashboard/assets/plugins/pace/pace.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="dashboard/assets/plugins/perfect-scrollbar/perfect-scrollbar.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="dashboard/assets/plugins/viewport/viewportchecker.js"><?php echo '</script'; ?>
>
    <!-- CORE JS FRAMEWORK - END -->

    <!-- OTHER SCRIPTS INCLUDED ON THIS PAGE - START -->
    <?php echo '<script'; ?>
 src="dashboard/assets/js/apexcharts.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="dashboard/assets/js/dash-scripts-2.js"><?php echo '</script'; ?>
>
    <!-- OTHER SCRIPTS INCLUDED ON THIS PAGE - END -->

    <!-- CORE TEMPLATE JS - START -->
    <?php echo '<script'; ?>
 src="dashboard/assets/js/scripts.js"><?php echo '</script'; ?>
>
    <!-- END CORE TEMPLATE JS - END -->


<?php echo '<script'; ?>
 type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js"><?php echo '</script'; ?>
>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">

    <!-- page-wrapper end -->
    <!-- jQuery library -->
    <?php echo '<script'; ?>
 src="assets/js/vendor/jquery-3.5.1.min.js"><?php echo '</script'; ?>
>
    <!-- bootstrap js -->
    <?php echo '<script'; ?>
 src="assets/js/vendor/bootstrap.bundle.min.js"><?php echo '</script'; ?>
>
    <!-- slick slider js -->
    <?php echo '<script'; ?>
 src="assets/js/vendor/slick.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="assets/js/vendor/wow.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="assets/js/contact.js"><?php echo '</script'; ?>
>
    <!-- dashboard custom js -->
    <?php echo '<script'; ?>
 src="assets/js/app.js"><?php echo '</script'; ?>
>
</body>
   </html>





<?php echo '<script'; ?>
 type="text/javascript" id="zsiqchat">var $zoho=$zoho || {};$zoho.salesiq = $zoho.salesiq || {widgetcode: "10a86fe0a4decf9bc15b5ae3675374c43a8268b2a38529b681cf2fba140fa6c3614d96f8c242795acf0632acf41b07fe", values:{},ready:function(){}};var d=document;s=d.createElement("script");s.type="text/javascript";s.id="zsiqscript";s.defer=true;s.src="https://salesiq.zoho.eu/widget";t=d.getElementsByTagName("script")[0];t.parentNode.insertBefore(s,t);<?php echo '</script'; ?>
>

<div class="fixed-to-bottom">
        <iframe class="ticker" src="http://www.exchangerates.org.uk/widget/ER-LRTICKER.php?w=2020&amp;s=1&amp;mc=USD&amp;mbg=F0F0F0&amp;bs=no&amp;bc=000014&amp;f=verdana&amp;fs=10px&amp;fc=000044&amp;lc=036EB4&amp;lhc=FE9A00&amp;vc=FE9A00&amp;vcu=008000&amp;vcd=0000&amp;" width="100%" height="30" frameborder="0" scrolling="no" marginwidth="0" marginheight="0"></iframe>
    </div>   	<?php }
}
?>